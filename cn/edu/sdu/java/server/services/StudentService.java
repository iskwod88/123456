...
public class StudentService {
    ...
    public StudentService(PersonRepository personRepository, StudentRepository studentRepository, UserRepository userRepository, UserTypeRepository userTypeRepository, PasswordEncoder encoder,  FeeRepository feeRepository, FamilyMemberRepository familyMemberRepository, SystemService systemService, CourseRepository courseRepository) {
    ...
    public DataResponse studentEditSave(DataRequest dataRequest) {
        ...
        if (nOp.isPresent()) {
            ...
            // 3.29：
            // 将  (s != null || !s.getPerson().getNum().equals(num))
            // 改为(s != null && s.getPerson().getNum().equals(num))
            // 失败
        }
        if (s == null) {
            p = new Person();
    ...
    //以下为添加课程选择：
    public Map<String, Object> getStudentCourseToMap(Course c) {
        Map<String, Object> m = new HashMap<>();
        if(c.getStudents() == null && c == null) {
            return null;
        }
        m.put("courseName", c.getName());
    ...
    public List<Map<String,Object>> getStudentCourseMapList(Student s) {
        List<Map<String,Object>> dataList = new ArrayList<>();
        List<Course> cList = new ArrayList<>();
        if(s == null || s.getCourses().isEmpty())
            return dataList;
        cList.addAll(s.getCourses());
        if (cList == null || cList.isEmpty())
            return dataList;
        for (Course course : cList) {
            dataList.add(getStudentCourseToMap(course));
        }
        return dataList;
    }
    ...
    public DataResponse getStudentCourseList(DataRequest dataRequest) {
        ...
        List<Map<String,Object>> dataList = getStudentCourseMapList(student.get());
        return CommonMethod.getReturnData(dataList);  //按照测试框架规范会送Map的list
    }

    public DataResponse addCourseToStudent(DataRequest studentRequest, DataRequest courseRequest) {
        Integer personId = studentRequest.getInteger("personId");
        Integer courseId = courseRequest.getInteger("courseId");
        if(personId == null || courseId == null)
                return CommonMethod.getReturnMessageError("该学生或课程不存在！");;
        ...
        if(student.isPresent() && course.isPresent()) {
            ...
            // 该句意为：对一个student对象的courses列表进行获取，之后用add添加获取到的指定课程到该列表中
            studentRepository.save(student.get());
        }
        return CommonMethod.getReturnMessageOK();
    }

    public DataResponse studentCourseDelete(DataRequest studentRequest, DataRequest courseRequest) {
        Integer personId = studentRequest.getInteger("personId");
        Integer courseId = courseRequest.getInteger("courseId");
        if(personId == null || personId <= 0 || courseId == null || courseId <= 0)
            return CommonMethod.getReturnMessageError("该学生或课程不存在！");

        Optional<Student> student = studentRepository.findById(personId);
        if (student.isPresent()) {
            student.get().getCourses().removeIf(c -> c.getCourseId().equals(courseId));
        }
